...
def create_order(customer_name: str, telegram_id: str, items: list, total: float,
...
def get_orders_by_user(telegram_id: str, business_id: int):
    response = (
...
def get_all_orders(business_id: int, status: str = None):
    query = (
        supabase.table("orders")
        .select("*")
        .eq("business_id", business_id)
...
def update_order_status(order_id: int, business_id: int, status: str):
    response = (
        supabase.table("orders")
